...
def main():
    ...
    # augment_type = args.train_dir.split("/")[-1].replace("train_", "")
    # save_dir = f"{args.save_dir}_sh-{args.schedule}_{augment_type}"
    ...
    # ... (existing imports and parser setup)

    args = parser.parse_args()

    # Change device to support multi-GPU
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(colored(
        f"Loading model: {args.model} on device: {device}",
        color="red", force_color=True
    ))
