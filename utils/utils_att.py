...
def validate(model, mha_block, dataloader, device):
    model.eval()
    mha_block.eval()
    all_preds = []
    all_labels = []
    ...
    # Calculate accuracy.
    accuracy = (torch.sum(predicts_tensor == labels_tensor).item() /
                len(labels_tensor))
    model.train()
    mha_block.train()
    return accuracy
...
def test(model, mha_block, dataloader, device, output_csv):
    model.eval()
    mha_block.eval()
    predictions = []
    ...
    print(f"Saved zipped predictions to {zip_filename}")
    model.train()
    mha_block.train()
